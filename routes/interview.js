...
router.get('/add',async (req,res)=>{
    ...
    try{
        ...
        admins = await User.find().sort({name:1}).select('name email phone');
    }catch(err){
    ...
    try{
        if(id && id.editID){
            ...
            if(inter.length==1){
                ...
                data.mode = 'Update';
                data.action = "/user/interview/add";
                data.method = "POST";
            }
        }
        debugEdit(data);
...
router.post('/add',async (req,res)=>{
    ...
    res.redirect('/user/interview/add');
});
...
function preProcess(data){
    ...
    if(data.admins.includes(data.users)){
        throw new Error("Select different people for the role of candidate and interviewer");
    }
...
async function validateTime(data){
    ...
    /*
    for(let i=0;i<data.admins.size;i++){
        if(data.interviewID)rows = await Interview.find({interviewer:{"$in":[data.admins[i]]} , _id:{$ne:data.interviewID}}).select('startTime endTime -_id');
        else rows = await Interview.find({interviewer:{"$in":[data.admins[i]]}}).select('startTime endTime');
        debug("all the rows - ", rows);
        for(x in rows){
            x = rows[x];
            if(!(x.startTime>data.endTime || x.endTime<data.startTime))throw new Error("Interview already scheduled for interview at that time");
        }
    }*/
    debug("Valid time!!");
